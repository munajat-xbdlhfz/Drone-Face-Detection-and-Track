...
class DroneManager(metaclass=Singleton):
    ...
    # Drone Flip Back Command
    def flip_back(self):
       return self.send_command('flip b')
    
    # Drone Flip Left Command
    def flip_left(self):
       return self.send_command('flip l')
    
    # Drone Flip Right Command
    def flip_right(self):
        ...
        for frame in self.video_binary_generator():
            if self._is_enable_face_detect:
                ...
                for (x, y, w, h) in faces:
                    ...
                    # Displaying Text of Face Recoqnize
                    id_, conf = recoqnizer.predict(roi_gray)
                    if conf >= 45:
